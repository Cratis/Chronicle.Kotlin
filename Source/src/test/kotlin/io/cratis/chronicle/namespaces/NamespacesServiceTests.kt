...
class NamespacesServiceTests {
    ...
    fun `getAll lists every namespace known to the event store`() = runBlocking {
        ...
        coEvery { stub.allNamespaces(capture(request), any()) } returns
            NamespacesOuterClass.QueryResult_IEnumerable_NamespaceNamesResponse.newBuilder()
                .addAllData(
                    listOf("Default", "tenant-a").map {
                        NamespacesOuterClass.NamespaceNamesResponse.newBuilder().setName(it).build()
                    }
                )
