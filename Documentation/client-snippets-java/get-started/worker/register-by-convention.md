```java
// Spring's own convention is @ComponentScan: every class carrying a recognized stereotype in the
// scanned packages becomes a registered bean automatically - the same mechanism that finds
...
class GetStartedWorkerConventionRegistration {
}
```
